'''
6/27/19
This script produces pretty results!!
I wanted to see if shifting continuously vs shifting discrete lpf cutoffs would make a difference.
It doesn't make any difference tbh. The models are trained over snrs in line 18, and the models are
called cont_ and disc_. Its a (32, 7) autoencoder with dropout rate 0, trained for 1000 epochs.
The plot is saved in presentation/disc_vs_cont_shifting
'''
import torch
...
for snr in snrs:
    com_dim = args.lpf_taps + args.channel_use - 1
    model = Net(channel_use=args.channel_use, block_size=args.block_size, snr=snr)
    model.load_state_dict(torch.load('./models/disc_' + str(snr)))
    model.eval()
    if USE_CUDA: model = model.cuda()
    test_out = model(test_data)
    pred = torch.round(test_out)
